...
function canary_deploy {
  ...
  kubectl scale deployment canary-v2 --replicas=2 #$((NUM_OF_V2_PODS + $DEPLOY_INCREMENTS))
  kubectl scale deployment canary-v1 --replicas=2 #$((NUM_OF_V1_PODS - $DEPLOY_INCREMENTS))
